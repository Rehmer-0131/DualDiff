...
def attack_local_models(args, protection):
    ...
    result_dir = args.protected_image_dir + '/' + \
        args.test_model_name[0] + '/' + args.target_choice +  '/' + f"step_{args.start_step}_iter{args.prot_steps}_lr_{args.lr_emb}_adv_{args.adv_optim_weight}_wav_{args.wavelet_weight}_reg_{args.reg_weight}_lpips_{args.lpips_weight}"
    ...
    if protection:
        for img_path in glob.glob(os.path.join(result_dir, "*.*")):
            ...
            # fn = fn = os.path.basename(img_path)
            # print(fn)
            ...
            clean_fn = os.path.splitext(fn)[0] + ".png" 
            
            clean_img = cv2.imread(os.path.join(args.source_dir, clean_fn))
